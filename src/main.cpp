...
int main(int argc, char* argv[]) {
  ...
  QSettings darkcrystalKey("HKEY_CLASSES_ROOT\\darkcrystal", QSettings::NativeFormat);
  darkcrystalKey.setValue(".", "DCRS Wallet");
  darkcrystalKey.setValue("URL Protocol", "");
  QSettings darkcrystalOpenKey("HKEY_CLASSES_ROOT\\darkcrystal\\shell\\open\\command", QSettings::NativeFormat);
  darkcrystalOpenKey.setValue(".", "\"" + QCoreApplication::applicationFilePath().replace("/", "\\") + "\" \"%1\"");
#endif
  ...
  //as root
  args << "-c" << "printf '[Desktop Entry]\\nName = DCRS URL Handler\\nGenericName = DCRS\\nComment = Handle URL Sheme darkcrystal://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/darkcrystal;\\nIcon = DCRS-Wallet' | tee /usr/share/applications/darkcrystal-handler.desktop";
