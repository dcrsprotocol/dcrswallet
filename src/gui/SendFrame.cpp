...
void SendFrame::parsePaymentRequest(QString _request) {
  ...
  if(_request.startsWith("darkcrystal://", Qt::CaseInsensitive))
  {
  	_request.replace(0, 13, "darkcrystal:");
  }
  if(!_request.startsWith("darkcrystal:", Qt::CaseInsensitive)) {
  	QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with darkcrystal:"), QtCriticalMsg));
    return;
  }
...
void SendFrame::sendClicked() {
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    if (m_ui->donateCheckBox->isChecked()) {
      CryptoNote::WalletLegacyTransfer walletTransfer;
      walletTransfer.address = "XDC1JovfkAn64t6iaN2mPKCVbZS5qE7AmiXaD6Bd8vdYQksJyiAcqWgLdF9F9QCytkYDsANhbaGXEExxYCHwEGvY8b7KiAaNav";
